...
namespace MATP {
    ...
    void MapManager::publish() {
        ...
        if(param.world_use_octomap and param.world_use_global_map){
            if(agent_frame_id == "mav0" and count_global_map_publish < 100){
    ...
    void MapManager::setGlobalMap() {
        if (has_global_map || !param.world_use_octomap || !param.world_use_global_map) {
            return;
        }
...
//    }
    ...
    void MapManager::updateOctreeFromCSV(){
        ...
        for(auto& row: CSVRange(obstacle_csv))
        {
            ...
            point3ds points(2); // center of mass, size
            for(int i = 0; i < 2; i++){
                for(int k = 0; k < 3; k++){
...
//            point3d start_point = points[0];
//            point3d end_point = points[1];
//
//            int i_start = (int)round(start_point.x() / param.world_resolution);
//            int i_end = (int)round(end_point.x() / param.world_resolution);
//            int j_start = (int)round(start_point.y() / param.world_resolution);
//            int j_end = (int)round(end_point.y() / param.world_resolution);
//            int k_start = (int)round(start_point.z() / param.world_resolution);
//            int k_end = (int)round(end_point.z() / param.world_resolution);
