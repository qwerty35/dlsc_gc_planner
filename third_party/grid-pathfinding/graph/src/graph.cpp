...
Grid::Grid(const Nodes& nodes, int width_, int depth_, int height_) : Graph() {
    width = width_;
    depth = depth_;
    height = height_;
    ...
    for (int z = 0; z < height; ++z) {
        for (int y = 0; y < depth; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                // left
                if (existNode(x - 1, y, z)) {
                    Node *w = getNode(x - 1, y, z);
                    if(v->warning || !w->warning){
                        v->neighbor.push_back(w);
                    }
                }
                // right
                if (existNode(x + 1, y, z)) {
                ...
                // up
                if (existNode(x, y - 1, z)) {
                    Node *w = getNode(x, y - 1, z);
                    if(v->warning || !w->warning){
                        v->neighbor.push_back(w);
                    }
                }
                ...
                // top
                if (existNode(x, y, z - 1)) {
                    Node *w = getNode(x, y, z - 1);
                    if(v->warning || !w->warning){
                        v->neighbor.push_back(w);
                    }
                }
